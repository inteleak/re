...
const DEFAULT_FALLBACK_TESTIMONIAL: Testimonial = {
  quote: "Mortanas yapay zeka otomasyon sistemleri sayesinde operasyonel verimliliğimiz inanılmaz derecede arttı. Kesinlikle çok profesyoneller.",
  name: "Caner Demir",
  title: "Yönetici, Vanguard Technologies",
  avatarUrl: "https://randomuser.me/api/portraits/men/32.jpg"
};

const TestimonialCard: React.FC<TestimonialCardProps> = ({ testimonial }) => {
  const activeTestimonial = testimonial || DEFAULT_FALLBACK_TESTIMONIAL;
  ...
  return (
    <div className="group bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950/90 backdrop-blur-2xl p-6 md:p-8 rounded-3xl shadow-[0_0_25px_rgba(168,85,247,0.1),_0_15px_40px_rgba(0,0,0,0.6)] border-2 border-purple-500/30 hover:border-purple-400 flex flex-col items-center text-center transition-all duration-500 hover:shadow-[0_0_40px_rgba(168,85,247,0.35)] hover:-translate-y-2 h-full relative overflow-hidden ring-4 ring-purple-500/5 hover:ring-purple-500/15">
      ...
      <img
        ...
        src={activeTestimonial.avatarUrl || "https://randomuser.me/api/portraits/men/32.jpg"}
      ...
      <div className="w-full mt-auto pt-4 border-t border-slate-800/80 relative z-10">
        <h4 className="font-bold text-base text-white group-hover:text-purple-300 transition-colors duration-300">{activeTestimonial.name}</h4>
        <p className="text-[11px] text-purple-400 uppercase tracking-wider font-bold mt-1">{activeTestimonial.title}</p>
      </div>
    </div>
  );
};
