...
function convertFiles(filename) {
    ...
    // Guest Journey Section Update
    const guestJourneyRegex = /<div className="relative z-10 w-full">\s*<h2 className="text-3xl md:text-4xl font-bold text-white">Misafir Yolculuğunun Her Adımında <span className="text-blue-400">Yanınızdayız<\/span><\/h2>\s*<p className="mt-4 text-lg text-slate-300 max-w-3xl mx-auto">\s*Platformumuz, misafirinizin otelinizle ilk temasından, sadık bir müşteriye dönüşmesine kadar tüm süreci akıllıca ve kesintisiz bir şekilde yönetir\.\s*<\/p>\s*<\/div>\s*<div className="relative">\s*<div className="hidden lg:block absolute top-12 left-0 w-full h-1 bg-slate-700" aria-hidden="true"><\/div>\s*<div className="relative grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-16">\s*\{guestJourneySteps\.map\(\(step, index\) => \([\s\S]*?\}\)\)\s*<\/div>\s*<\/div>/g;
    ...
    content = content.replace(guestJourneyRegex, newGuestJourney);
